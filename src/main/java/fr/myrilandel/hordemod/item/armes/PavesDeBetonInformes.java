...
public class PavesDeBetonInformes extends Item{
	public IIcon[] icons = new IIcon[2];

	public static boolean critSuccess = true;
	public PavesDeBetonInformes() {
		...
		if(!entityPlayer.worldObj.isRemote){
			...
			if(!critSuccess) {
				if(Math.random()>0.50)
					entity.attackEntityFrom(DamageSource.cactus, 50.0F);
				if(Math.random()>0.30) 
					entityPlayer.inventory.setInventorySlotContents(entityPlayer.inventory.currentItem,new ItemStack(HMItemRegistry.pavesDeBetonInformesCasses));
			}
			switchCritSuccess();
		}
		return false;
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < icons.length; i ++) {
	        this.icons[i] = reg.registerIcon(HordeMod.MODID + ":pavesDeBetonInformes");
	    }
	}
