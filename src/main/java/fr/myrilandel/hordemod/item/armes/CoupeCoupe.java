...
public class CoupeCoupe extends Item {
	...
	public CoupeCoupe() {
		...
		this.setMaxDamage(0);
		this.setUnlocalizedName("CoupeCoupe");
		this.setTextureName(HordeMod.MODID + ":coupeCoupe");
	...
	public boolean onLeftClickEntity(ItemStack itemStack, EntityPlayer entityPlayer, Entity entity) {
		if(!entityPlayer.worldObj.isRemote){
			...
			if(!critSuccess) 
				if(Math.random()>0.75) 
					entityPlayer.inventory.setInventorySlotContents(entityPlayer.inventory.currentItem,new ItemStack(HMItemRegistry.coupeCoupeCasse));
			switchCritSuccess();
		}
		return false;
	}
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < icons.length; i ++) {
	        this.icons[i] = reg.registerIcon(HordeMod.MODID + ":coupeCoupe");
	    }
	}
