...
public class UniteCentrale extends Item{
	public IIcon[] icons = new IIcon[2];

	public UniteCentrale() {
		this.setHasSubtypes(true);
		this.setMaxDamage(0);
		this.setUnlocalizedName("UniteCentrale");
		this.setTextureName(HordeMod.MODID + ":uniteCentrale");
	...
	public boolean onLeftClickEntity(ItemStack itemStack, EntityPlayer entityPlayer, Entity entity) {
		if(!entityPlayer.worldObj.isRemote){
			...
			if(Math.random()>0.45) 
				entityPlayer.inventory.setInventorySlotContents(entityPlayer.inventory.currentItem,new ItemStack(HMItemRegistry.uniteCentraleCassee));
		}
		return false;
	}
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < icons.length; i ++) {
	        this.icons[i] = reg.registerIcon(HordeMod.MODID + ":uniteCentrale");
	    }
	}
