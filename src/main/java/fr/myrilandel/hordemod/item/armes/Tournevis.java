...
public class Tournevis extends Item {
	public IIcon[] icons = new IIcon[2];

	public Tournevis() {
		this.setHasSubtypes(true);
		this.setMaxDamage(0);
		this.setUnlocalizedName("Tournevis");
		this.setTextureName(HordeMod.MODID + ":tournevis");
	...
	public boolean onLeftClickEntity(ItemStack itemStack, EntityPlayer entityPlayer, Entity entity) {
		if(!entityPlayer.worldObj.isRemote){
			...
			if(Math.random()>0.50) 
				entityPlayer.inventory.setInventorySlotContents(entityPlayer.inventory.currentItem,new ItemStack(HMItemRegistry.tournevisCasse));
		}
		return false;
	}
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < icons.length; i ++) {
	        this.icons[i] = reg.registerIcon(HordeMod.MODID + ":tournevis");
	    }
	}
