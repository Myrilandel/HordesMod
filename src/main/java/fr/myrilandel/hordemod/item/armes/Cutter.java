...
public class Cutter extends Item{
	...
	public Cutter() {
		...
		this.setMaxDamage(0);
		this.setUnlocalizedName("Cutter");
		this.setTextureName(HordeMod.MODID + ":cutter");
	...
	public boolean onLeftClickEntity(ItemStack itemStack, EntityPlayer entityPlayer, Entity entity) {
		if(!entityPlayer.worldObj.isRemote){
			...
			if(Math.random()>0.10) 
				entityPlayer.inventory.setInventorySlotContents(entityPlayer.inventory.currentItem,new ItemStack(HMItemRegistry.cutterCasse));
		}
		return false;
	}
	...
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < icons.length; i ++) {
	        this.icons[i] = reg.registerIcon(HordeMod.MODID + ":cutter");
	    }
	}
