...
public class CoffreDArchitecteScelle extends Item{

	public CoffreDArchitecteScelle() {
		this.setUnlocalizedName("CoffreDArchitecteScelle");
		this.setTextureName(HordeMod.MODID + ":coffreDArchitecteScelle");
		...
		if(!world.isRemote) {
			...
			if(EntityPropPA.pertePA(10))
				if(Math.random() > 0.9) {
					...
					return new ItemStack(HMItemRegistry.coffreDArchitecteOuvert);
				}
				else {
