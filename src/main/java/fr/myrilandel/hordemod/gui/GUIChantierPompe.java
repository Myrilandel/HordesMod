...
public class GUIChantierPompe extends GuiContainer {

	World world;
	TileEntityChantierPompe TEPompe;
	...
	public GUIChantierPompe(World world, TileEntityChantierPompe tileEntityChantierPompe, TileEntityBanque tileEntityBanque,
			TileEntityBlocksVille tileEntityBlocksVille, EntityPlayer player) {
		super(new ContainerChantierPompe(tileEntityChantierPompe, world));
		this.world = world;
		this.TEPompe = tileEntityChantierPompe;
	...
	protected void actionPerformed(GuiButton button) {
		if (button.enabled) {
			props.pertePA(1);
			HMPacketHelper.sendPacket(TEPompe, button.id);
			HMPacketHelper.sendPacket(tileEntityBanque, button.id,1);
			HMPacketHelper.sendPacket(TEVille, button.id,1);
		}
	}
	...
	private void drawChantierContainer(int x, int y) {
		...
		drawLigneChantier(x, y, 1, "Muraille", ChantiersRegistry.defMuraille, ChantiersRegistry.recetteMuraille,
				ChantiersRegistry.texturesMuraille, new int[]{nbPlanchesTordue,nbFerraille}, TEPompe.PAMuraille);
		if(TEPompe.PAMuraille == 0)
	...
	private void updateButtons() {
		...
		for (Object object : buttonList) 
			if(object instanceof GuiBooleanButton) {
				...
				switch (button.id) {
				case 1:button.visible = TEPompe.PAMuraille>0;button.enabled = checkButtonEnabled(new int[]{nbPlanchesTordue, nbFerraille}, ChantiersRegistry.recetteMuraille);break;
	...
	private boolean checkButtonVisible(int ID) {
		...
		if(TEPompe.PAMuraille == 0) {
			switch (ID) {
			case 2: return TEPompe.PAGrandFosse != 0;
			case 3: return TEPompe.PADouves != 0 && TEPompe.PAGrandFosse == 0 && TEVille.tabCommunIsUnlocked[0];
			case 4: return TEPompe.PAMurailleRasoir != 0 && TEVille.tabCommunIsUnlocked[1];
			case 5: return TEPompe.PAFosseAPieux != 0 && TEVille.tabCommunIsUnlocked[2];
			case 6: return TEPompe.PABarbeles != 0;
			case 7: return TEPompe.PAAppats != 0 && TEPompe.PABarbeles == 0;
			case 8: return TEPompe.PARempartsAvances != 0 && TEVille.tabCommunIsUnlocked[3];
			case 9: return TEPompe.PAPoutresRenfort != 0;
			case 10: return TEPompe.PAMurailleAPointes != 0 && TEPompe.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[4];
			case 11: return TEPompe.PAGrogroMur != 0 && TEPompe.PAMurailleAPointes == 0 && TEVille.tabInhabituelIsUnlocked[0];
			case 12: return TEPompe.PASecondeCouche != 0 && TEPompe.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[5];
			case 13: return TEPompe.PATroisièmeCouche != 0 && TEPompe.PASecondeCouche == 0 && TEVille.tabInhabituelIsUnlocked[1];
			case 14: return TEPompe.PAMurailleEvolutive != 0 && TEPompe.PAPoutresRenfort == 0 && TEVille.tabRareIsUnlocked[0];
			case 15: return TEPompe.PABetonnade != 0 && TEPompe.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[6];
			case 16: return TEPompe.PARapeAZombies != 0 && TEVille.tabCommunIsUnlocked[7];
			case 17: return TEPompe.PAOubliettes != 0;
			case 18: return TEPompe.PABarrieres != 0;
			case 19: return TEPompe.PAPalissade != 0 && TEVille.tabCommunIsUnlocked[8];
			case 20: return TEPompe.PAMurSavone != 0 && TEVille.tabCommunIsUnlocked[9];
			case 21: return TEPompe.PAPulverisateur != 0 && TEVille.tabCommunIsUnlocked[10];
			case 22: return TEPompe.PAProjectionAcide != 0 && TEPompe.PAPulverisateur == 0 && TEVille.tabCommunIsUnlocked[11];
			case 23: return TEPompe.PANeurotoxine != 0 && TEPompe.PAPulverisateur == 0 && TEVille.tabInhabituelIsUnlocked[2];
			case 24: return TEPompe.PACloisonEnBois != 0;
			case 25: return TEPompe.PACloisonMetallique != 0;
			case 26: return TEPompe.PACloisonEpaisse != 0;
			case 27: return TEPompe.PAContrePlaque != 0;
			case 28: return TEPompe.PABastion != 0 && TEVille.tabCommunIsUnlocked[12];
			default:return true;
			}
		}
		return false;
