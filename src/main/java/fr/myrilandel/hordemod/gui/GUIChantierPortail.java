...
public class GUIChantierPortail extends GuiContainer {

	World world;
	TileEntityChantierPortail TEPortail;
	...
	public GUIChantierPortail(World world, TileEntityChantierPortail tileEntityChantierPortail, TileEntityBanque tileEntityBanque,
			TileEntityBlocksVille tileEntityBlocksVille, EntityPlayer player) {
		super(new ContainerChantierPortail(tileEntityChantierPortail, world));
		this.world = world;
		this.TEPortail = tileEntityChantierPortail;
	...
	protected void actionPerformed(GuiButton button) {
		if (button.enabled) {
			props.pertePA(1);
			HMPacketHelper.sendPacket(TEPortail, button.id);
			HMPacketHelper.sendPacket(tileEntityBanque, button.id,5);
			HMPacketHelper.sendPacket(TEVille, button.id,5);
		}
	}
	...
	private void drawChantierContainer(int x, int y) {
		...
		drawLigneChantier(x, y, 1, "Muraille", ChantiersRegistry.defMuraille, ChantiersRegistry.recetteMuraille,
				ChantiersRegistry.texturesMuraille, new int[]{nbPlanchesTordue,nbFerraille}, TEPortail.PAMuraille);
		if(TEPortail.PAMuraille == 0)
	...
	private void updateButtons() {
		...
		for (Object object : buttonList) 
			if(object instanceof GuiBooleanButton) {
				...
				switch (button.id) {
				case 1:button.visible = TEPortail.PAMuraille>0;button.enabled = checkButtonEnabled(new int[]{nbPlanchesTordue, nbFerraille}, ChantiersRegistry.recetteMuraille);break;
	...
	private boolean checkButtonVisible(int ID) {
		...
		if(TEPortail.PAMuraille == 0) {
			switch (ID) {
			case 2: return TEPortail.PAGrandFosse != 0;
			case 3: return TEPortail.PADouves != 0 && TEPortail.PAGrandFosse == 0 && TEVille.tabCommunIsUnlocked[0];
			case 4: return TEPortail.PAMurailleRasoir != 0 && TEVille.tabCommunIsUnlocked[1];
			case 5: return TEPortail.PAFosseAPieux != 0 && TEVille.tabCommunIsUnlocked[2];
			case 6: return TEPortail.PABarbeles != 0;
			case 7: return TEPortail.PAAppats != 0 && TEPortail.PABarbeles == 0;
			case 8: return TEPortail.PARempartsAvances != 0 && TEVille.tabCommunIsUnlocked[3];
			case 9: return TEPortail.PAPoutresRenfort != 0;
			case 10: return TEPortail.PAMurailleAPointes != 0 && TEPortail.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[4];
			case 11: return TEPortail.PAGrogroMur != 0 && TEPortail.PAMurailleAPointes == 0 && TEVille.tabInhabituelIsUnlocked[0];
			case 12: return TEPortail.PASecondeCouche != 0 && TEPortail.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[5];
			case 13: return TEPortail.PATroisièmeCouche != 0 && TEPortail.PASecondeCouche == 0 && TEVille.tabInhabituelIsUnlocked[1];
			case 14: return TEPortail.PAMurailleEvolutive != 0 && TEPortail.PAPoutresRenfort == 0 && TEVille.tabRareIsUnlocked[0];
			case 15: return TEPortail.PABetonnade != 0 && TEPortail.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[6];
			case 16: return TEPortail.PARapeAZombies != 0 && TEVille.tabCommunIsUnlocked[7];
			case 17: return TEPortail.PAOubliettes != 0;
			case 18: return TEPortail.PABarrieres != 0;
			case 19: return TEPortail.PAPalissade != 0 && TEVille.tabCommunIsUnlocked[8];
			case 20: return TEPortail.PAMurSavone != 0 && TEVille.tabCommunIsUnlocked[9];
			case 21: return TEPortail.PAPulverisateur != 0 && TEVille.tabCommunIsUnlocked[10];
			case 22: return TEPortail.PAProjectionAcide != 0 && TEPortail.PAPulverisateur == 0 && TEVille.tabCommunIsUnlocked[11];
			case 23: return TEPortail.PANeurotoxine != 0 && TEPortail.PAPulverisateur == 0 && TEVille.tabInhabituelIsUnlocked[2];
			case 24: return TEPortail.PACloisonEnBois != 0;
			case 25: return TEPortail.PACloisonMetallique != 0;
			case 26: return TEPortail.PACloisonEpaisse != 0;
			case 27: return TEPortail.PAContrePlaque != 0;
			case 28: return TEPortail.PABastion != 0 && TEVille.tabCommunIsUnlocked[12];
			default:return true;
			}
		}
		return false;
