...
public class GUIChantierTDG extends GuiContainer {

	World world;
	TileEntityChantierTDG TETDG;
	...
	public GUIChantierTDG(World world, TileEntityChantierTDG tileEntityChantierMuraille, TileEntityBanque tileEntityBanque,
			TileEntityBlocksVille tileEntityBlocksVille, EntityPlayer player) {
		super(new ContainerChantierTDG(tileEntityChantierMuraille, world));
		this.world = world;
		this.TETDG = tileEntityChantierMuraille;
	...
	protected void actionPerformed(GuiButton button) {
		if (button.enabled) {
			props.pertePA(1);
			HMPacketHelper.sendPacket(TETDG, button.id);
			HMPacketHelper.sendPacket(tileEntityBanque, button.id,3);
			HMPacketHelper.sendPacket(TEVille, button.id,3);
		}
	}
	...
	private void drawChantierContainer(int x, int y) {
		...
		drawLigneChantier(x, y, 1, "Muraille", ChantiersRegistry.defMuraille, ChantiersRegistry.recetteMuraille,
				ChantiersRegistry.texturesMuraille, new int[]{nbPlanchesTordue,nbFerraille}, TETDG.PAMuraille);
		if(TETDG.PAMuraille == 0)
	...
	private void updateButtons() {
		...
		for (Object object : buttonList) 
			if(object instanceof GuiBooleanButton) {
				...
				switch (button.id) {
				case 1:button.visible = TETDG.PAMuraille>0;button.enabled = checkButtonEnabled(new int[]{nbPlanchesTordue, nbFerraille}, ChantiersRegistry.recetteMuraille);break;
	...
	private boolean checkButtonVisible(int ID) {
		...
		if(TETDG.PAMuraille == 0) {
			switch (ID) {
			case 2: return TETDG.PAGrandFosse != 0;
			case 3: return TETDG.PADouves != 0 && TETDG.PAGrandFosse == 0 && TEVille.tabCommunIsUnlocked[0];
			case 4: return TETDG.PAMurailleRasoir != 0 && TEVille.tabCommunIsUnlocked[1];
			case 5: return TETDG.PAFosseAPieux != 0 && TEVille.tabCommunIsUnlocked[2];
			case 6: return TETDG.PABarbeles != 0;
			case 7: return TETDG.PAAppats != 0 && TETDG.PABarbeles == 0;
			case 8: return TETDG.PARempartsAvances != 0 && TEVille.tabCommunIsUnlocked[3];
			case 9: return TETDG.PAPoutresRenfort != 0;
			case 10: return TETDG.PAMurailleAPointes != 0 && TETDG.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[4];
			case 11: return TETDG.PAGrogroMur != 0 && TETDG.PAMurailleAPointes == 0 && TEVille.tabInhabituelIsUnlocked[0];
			case 12: return TETDG.PASecondeCouche != 0 && TETDG.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[5];
			case 13: return TETDG.PATroisièmeCouche != 0 && TETDG.PASecondeCouche == 0 && TEVille.tabInhabituelIsUnlocked[1];
			case 14: return TETDG.PAMurailleEvolutive != 0 && TETDG.PAPoutresRenfort == 0 && TEVille.tabRareIsUnlocked[0];
			case 15: return TETDG.PABetonnade != 0 && TETDG.PAPoutresRenfort == 0 && TEVille.tabCommunIsUnlocked[6];
			case 16: return TETDG.PARapeAZombies != 0 && TEVille.tabCommunIsUnlocked[7];
			case 17: return TETDG.PAOubliettes != 0;
			case 18: return TETDG.PABarrieres != 0;
			case 19: return TETDG.PAPalissade != 0 && TEVille.tabCommunIsUnlocked[8];
			case 20: return TETDG.PAMurSavone != 0 && TEVille.tabCommunIsUnlocked[9];
			case 21: return TETDG.PAPulverisateur != 0 && TEVille.tabCommunIsUnlocked[10];
			case 22: return TETDG.PAProjectionAcide != 0 && TETDG.PAPulverisateur == 0 && TEVille.tabCommunIsUnlocked[11];
			case 23: return TETDG.PANeurotoxine != 0 && TETDG.PAPulverisateur == 0 && TEVille.tabInhabituelIsUnlocked[2];
			case 24: return TETDG.PACloisonEnBois != 0;
			case 25: return TETDG.PACloisonMetallique != 0;
			case 26: return TETDG.PACloisonEpaisse != 0;
			case 27: return TETDG.PAContrePlaque != 0;
			case 28: return TETDG.PABastion != 0 && TEVille.tabCommunIsUnlocked[12];
			default:return true;
			}
		}
		return false;
